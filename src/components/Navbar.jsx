    ...
    >
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8 lg:py-5">
        ...
        <nav aria-label="Primary navigation" className="flex flex-wrap items-center justify-center gap-3 text-sm text-slate-300 sm:gap-5 lg:gap-7">
          {navItems.map((item) => (
            <a key={item.label} href={item.href} className="rounded-full px-2 py-1 transition hover:text-white">
              {item.label}
            </a>
          ))}
        </nav>
        ...
        <a
          href="#dashboard"
          className="inline-flex self-center rounded-full border border-emerald-400/30 bg-emerald-400/10 px-4 py-2 text-sm font-medium text-emerald-300 transition hover:bg-emerald-400/20 lg:self-auto"
        >
          Explore
        </a>
      </div>
